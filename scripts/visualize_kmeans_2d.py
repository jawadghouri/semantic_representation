...
def visualize_2d_kmeans_by_cluster(embeddings_dict, metadata, project_root):
    """Create 2D PCA visualizations colored by K-means clusters (normalized vs unnormalized)."""
    output_dir = project_root / "new/visualizations"
    output_dir.mkdir(parents=True, exist_ok=True)

    print("\n" + "="*80)
    print("CREATING 2D K-MEANS VISUALIZATIONS - COLORED BY CLUSTER")
    print("="*80)

    embedding_models = ['bge', 'e5', 'minilm']
    k_value = 5  # Use K=5 for better visualization (less crowded than K=10)

    # Create figure: 3 embedding models × 2 normalization types
    fig = plt.figure(figsize=(20, 10))
    fig.suptitle(f'2D PCA with K-Means Clustering (K={k_value}): Normalized vs Unnormalized',
    ...
    for model_idx, model_name in enumerate(embedding_models, 1):
        ...
        ax_unnorm.scatter(proj_unnorm[:, 0], proj_unnorm[:, 1],
                         c=labels_unnorm, cmap='tab10', s=120,
        ...
        ax_norm.scatter(proj_norm[:, 0], proj_norm[:, 1],
                       c=labels_norm, cmap='tab10', s=120,
                       alpha=0.7, edgecolors='black', linewidth=0.5)
    ...
    plt.tight_layout()
    viz_file = output_dir / f"kmeans_2d_clusters_k{k_value}.png"
    plt.savefig(viz_file, dpi=150, bbox_inches='tight')
    print(f"✓ Saved: {viz_file.name}")
    plt.close()

def visualize_2d_kmeans_by_llm(embeddings_dict, metadata, project_root):
    """Create 2D PCA with K-means clusters, colored by LLM model."""
    output_dir = project_root / "new/visualizations"

    print("\n" + "="*80)
    print("CREATING 2D K-MEANS VISUALIZATIONS - COLORED BY LLM MODEL (with cluster borders)")
    print("="*80)
    ...
    # Create figure: 3 embedding models × 2 normalization types
    fig = plt.figure(figsize=(20, 10))
    fig.suptitle(f'2D PCA: LLM Models + K-Means Clustering (K={k_value}): Normalized vs Unnormalized',
                fontsize=16, fontweight='bold')
    ...
    for model_idx, model_name in enumerate(embedding_models, 1):
        ...
        ax_unnorm.scatter(pca_centers[:, 0], pca_centers[:, 1],
        ...
        ax_norm.scatter(pca_centers[:, 0], pca_centers[:, 1],
                       c='yellow', marker='*', s=500, edgecolors='black', linewidth=2,
                       label='Centroids', zorder=5)
    ...
    plt.tight_layout()
    viz_file = output_dir / f"kmeans_2d_by_llm_k{k_value}.png"
    plt.savefig(viz_file, dpi=150, bbox_inches='tight')
    print(f"✓ Saved: {viz_file.name}")
    plt.close()
...
def main():
    ...
    visualize_2d_kmeans_by_cluster(embeddings_dict, metadata, project_root)
    visualize_2d_kmeans_by_llm(embeddings_dict, metadata, project_root)

    print("\n" + "="*80)
    print("✓ 2D K-MEANS VISUALIZATIONS CREATED")
    print("="*80)
