...
def create_3d_comparison_by_llm(embeddings_dict, metadata, project_root):
    """Create 3D PCA comparison (normalized vs unnormalized) colored by LLM model."""
    output_dir = project_root / "new/visualizations"
    output_dir.mkdir(parents=True, exist_ok=True)

    print("\n" + "="*80)
    print("CREATING 3D PCA COMPARISON - LLM-CENTRIC (NORMALIZED vs UNNORMALIZED)")
    print("="*80)
    ...
    fig.suptitle('3D PCA Comparison: Normalized vs Unnormalized (Colored by LLM Model)',
    ...
    plt.tight_layout()
    viz_file = output_dir / "pca_3d_comparison_by_llm.png"
    plt.savefig(viz_file, dpi=150, bbox_inches='tight')
    print(f"✓ Saved: {viz_file.name}")
    plt.close()

def create_3d_comparison_by_embedding_model(embeddings_dict, metadata, project_root):
    """Create 3D PCA comparison (normalized vs unnormalized) by embedding model."""
    output_dir = project_root / "new/visualizations"

    print("\n" + "="*80)
    print("CREATING 3D PCA COMPARISON - EMBEDDING MODEL (NORMALIZED vs UNNORMALIZED)")
    print("="*80)
    ...
    # Create figure: 3 embedding models × 2 normalization types
    fig = plt.figure(figsize=(24, 12))
    fig.suptitle('3D PCA Comparison: Normalized vs Unnormalized (Embedding Model View)',
                fontsize=16, fontweight='bold')
    ...
    for model_idx, model_name in enumerate(embedding_models, 1):
        ...
        ax_unnorm.scatter(proj_unnorm[:, 0], proj_unnorm[:, 1], proj_unnorm[:, 2],
        ...
        ax_unnorm.set_title(f'{model_name.upper()} - UNNORMALIZED\nVar: {pca_unnorm.explained_variance_ratio_.sum()*100:.1f}%',
                           fontsize=11, fontweight='bold', color='darkred')
        ax_unnorm.view_init(elev=20, azim=45)
        ax_unnorm.legend(fontsize=8, loc='upper left')
        ...
        ax_norm.scatter(proj_norm[:, 0], proj_norm[:, 1], proj_norm[:, 2],
                       c=color, label=f'{model_name.upper()} Responses',
                       s=80, alpha=0.7, edgecolors='black', linewidth=0.5)
    ...
    plt.tight_layout()
    viz_file = output_dir / "pca_3d_comparison_by_embedding_model.png"
    plt.savefig(viz_file, dpi=150, bbox_inches='tight')
    print(f"✓ Saved: {viz_file.name}")
    plt.close()
...
def main():
    ...
    create_3d_comparison_by_llm(embeddings_dict, metadata, project_root)
    create_3d_comparison_by_embedding_model(embeddings_dict, metadata, project_root)

    print("\n" + "="*80)
    print("✓ 3D PCA COMPARISON VISUALIZATIONS CREATED")
    print("="*80)
