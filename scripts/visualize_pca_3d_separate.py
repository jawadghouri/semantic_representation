...
def create_3d_pca_by_llm(embeddings_dict, metadata, project_root):
    """Create separate 3D PCA plot colored by LLM model."""
    output_dir = project_root / "new/visualizations"
    output_dir.mkdir(parents=True, exist_ok=True)

    print("\n" + "="*80)
    print("CREATING 3D PCA - LLM-CENTRIC ANALYSIS")
    print("="*80)
    ...
    fig.suptitle('3D PCA Projections - Colored by LLM Model',
...
def create_3d_pca_by_embedding_model(embeddings_dict, metadata, project_root):
    """Create separate 3D PCA plot showing embedding model comparison."""
    output_dir = project_root / "new/visualizations"

    print("\n" + "="*80)
    print("CREATING 3D PCA - EMBEDDING MODEL COMPARISON")
    print("="*80)
    ...
    # Create figure with 3 subplots (one for each embedding model)
    fig = plt.figure(figsize=(20, 6))
    fig.suptitle('3D PCA - Embedding Model Comparison',
                fontsize=14, fontweight='bold')
