...
def assign_cluster_labels(labels, metadata):
    """Assign LLM names to clusters based on dominant LLM in each cluster."""
    k_value = len(set(labels))
    llm_names = sorted(set(m['model'] for m in metadata))
    ...
    for cluster_id in range(k_value):
        ...
        # Count LLM occurrences in this cluster
        llm_counts = {}
        ...
        # Find dominant LLM
        dominant_llm = max(llm_counts, key=llm_counts.get) if llm_counts else "Unknown"
        cluster_to_llm[cluster_id] = dominant_llm

    return cluster_to_llm

def create_before_after_with_llm_labels(embeddings_dict, metadata, project_root):
    """Create Before/After K-Means plots with LLM labels."""
    output_dir = project_root / "new/visualizations/kmeans_clustering"
    output_dir.mkdir(parents=True, exist_ok=True)

    print("\n" + "="*80)
    print("CREATING BEFORE/AFTER K-MEANS PLOTS (K=5 with LLM Labels)")
    print("="*80)
    ...
    for model_name in embedding_models:
        ...
        # Assign LLM names to clusters
        cluster_to_llm = assign_cluster_labels(labels, metadata)
        print(f"  Cluster assignments: {cluster_to_llm}")
        ...
        # Plot clusters with LLM color mapping and labels
        for cluster_id in range(k_value):
            cluster_mask = labels == cluster_id
        ...
        ax_after.legend(fontsize=12, loc='best', framealpha=0.9, title='LLM Cluster')
...
def main():
    ...
    # Create visualizations with LLM labels
    create_before_after_with_llm_labels(embeddings_dict, metadata, project_root)

    print("\n" + "="*80)
    print("✓ BEFORE/AFTER K=5 PLOTS WITH LLM LABELS CREATED")
    print("="*80)
