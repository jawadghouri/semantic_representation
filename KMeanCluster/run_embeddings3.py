...
for group_key, filepath in model_files.items():
    ...
    # ---------------------------------------------------
    # RUN NORMALIZED EMBEDDINGS
    # ---------------------------------------------------
    for embed_name, embedder in embedders.items():
        print(f"Embedding with: {embed_name} (NORMALIZED)")
        embeddings_norm = embedder.encode(texts, normalize_embeddings=True)

        for text_id, single_embedding in zip(ids, embeddings_norm):
            output_path = f"data/processed/embeddings_norm/{text_id}_{embed_name}_norm.npy"
            os.makedirs(os.path.dirname(output_path), exist_ok=True)
            save_numpy(single_embedding, output_path)

check_embeddings_normalization("data/processed/embeddings_unnorm/")
check_embeddings_normalization("data/processed/embeddings_norm/")
